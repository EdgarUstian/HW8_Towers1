...
class Game {
  ...
  def checkForBaseDamage(): Unit = {
    // TODO: Objective 1
    val baseLocation = new PhysicsVector(level.base.x + 0.5, level.base.y + 0.5)
    for ((name, player) <- players){
      if (baseLocation.distance2d(player.location) < playerSize){
        baseHealth -= 1
  ...
  def checkForPlayerHits(): Unit = {
    ...
    for ((name, player) <- players){
      for (projectile <- projectiles){
        if (projectile.location.distance2d(player.location) < playerSize){
          projectile.destroy()
          player.location = startingVector()
        }
      }
